...
def address2(request):        
    ...
    action="electricity"
    context = {'form':form, 'links':links, 'heading':heading, 'subheading':subheading, 'note':note, 'img_url':img_url,'action':action, 'section_id':section_id, 'address':a}
...
def installation(request):
    ...
    if request.method == 'POST':
        ...
        if 'installer' in request.POST:
            y=request.POST.get('installer')
        ...
        #this is triggered when house picker is used, and list of installers not yet generated
            ...
            #save the selected date to the house database -can be changed later on reselect
            instance.receptacle_installation_date=x
            instance.save()
            #re-render the installation page with list of available installers
            context = {'house_availability_form':house_availability_form, 'receptacle_installer_list':receptacle_installer_list, 'text':text, 'header_text': header_text, 'action':action, 'note':note}
...
def roof_installation(request):
    house_availability_form=House_Availability_Form()
    ...
    note="This would work like previous form."
    href="/appliance_installation"
...
def appliance_installation(request):
    ...
    text="Select a date for your appliance installation."
    action="appliance_installation"
    note="[Note: This would work like previous form.]"
    href="/purchase"
    context = {'house_availability_form':house_availability_form, 'text':text, 'header_text': header_text, 'action':action, 'note':note, 'href':href}
    return render(request,'buy/installation.html',context)  
